...
def compute_spherical_string_grad_single_trg(pt1, pt2, pt3, bary, orient, wv):
    A = compute_spherical_trg_area(pt1, pt2, pt3, bary) / wv
    if A == 4*np.pi:
        if orient == 1:
            g1 = A*np.array([0, 1, 0])
...
def compute_spherical_spring_grad_single_trg(pt1, pt2, pt3, bary, orient, wv, S, K):
    if (pt1 == pt2).all():
        if (pt1 == pt3).all():
            if pt1.dot(bary) > 0:
                    D = K*S
...
def compute_grad_and_NRG(pts, trg_list, bary, orient, E_type, weight_values, S=0, K=1):
    ...
    for i in range(n_trg):
        ...
        if E_type == 0:
            g1, g2, g3, A = compute_spherical_string_grad_single_trg(pt1, pt2, pt3, bary[i], orient[i], weight_values[i])
        elif E_type == 1:
            g1, g2, g3, A = compute_spherical_spring_grad_single_trg(pt1, pt2, pt3, bary[i], orient[i], weight_values[i], S, K)
...
def dim_2_get_edges_and_cohomology_info(distance_matrix, selected_cocycle, max_nhood_size, weighting = 'triangles'):
    all_2_simplices = []
    all_values = []
    all_weight_values = []
    ...
    if weighting == 'triangles':
        for i in range(num_points):
            for j in range(num_points):
                # Check if the pair (i, j) is close enough
                if distance_matrix[i, j] <= max_nhood_size:
                    # Initialize the counter for this edge (i, j)
                    dic_weight[(i, j)] = 0
                    
                    for k in range(num_points):
                        if distance_matrix[i, k] <= max_nhood_size and distance_matrix[j, k] <= max_nhood_size:
                            dic_weight[(i, j)] += 1
    else:
        for i in range(num_points):
            dic_weight[i] = 0
                ...
                if distance_matrix[i, j] <= max_nhood_size and distance_matrix[i, k] <= max_nhood_size and \
                    ...
                    for [v_1, v_2, v_3, val] in selected_cocycle:
                        if i == v_1 and j == v_2 and k == v_3:
                            value = value + val
                    if weighting == 'triangles':
                        w_value = (dic_weight[(i,j)] + dic_weight[(j,k)] + dic_weight[(i,k)])
                    else:
                        w_value = (dic_weight[i] + dic_weight[j] + dic_weight[i])
                    all_values.append(value)
                    all_weight_values.append(w_value)
    return all_2_simplices, all_values, all_weight_values
...
def generate_density_sphere(num_points = 50, radius = 1, noise = 0., angle = np.pi/4):
    random_float_array = np.random.normal(0, angle, (num_points, 2))
    res = np.random.uniform(-1.0, 1.0, [2*num_points, 3])
    for i in range(num_points):
        norm = radius + noise*np.random.randn()
        res[i][0] = norm * np.sin(random_float_array[i][0]) * np.cos(random_float_array[i][1])
        res[i][1] = norm * np.sin(random_float_array[i][0]) * np.sin(random_float_array[i][1]) 
        res[i][2] = norm * np.cos(random_float_array[i][0])
        norm = np.sqrt(res[i+num_points][0]**2 + res[i+num_points][1]**2 + res[i+num_points][2]**2)
        for j in range(3):
            res[i][j] = (radius+noise*np.random.randn())*(res[i+num_points][j]/norm) 
    return res
